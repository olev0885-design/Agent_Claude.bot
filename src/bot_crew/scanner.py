...
class FundingScanner:
    ...
    def _in_open_grace_period(self, coin: str) -> bool:
        """True, если по этой монете НЕДАВНО стартовало открытие ИЛИ ЗАКРЫТИЕ
        (см. self._recent_open_attempts в __init__ — с 2026-09-15 туда же
        пишет и _close_test_batch_position) — reconcile должен ПОЛНОСТЬЮ
        пропустить монету на этом прогоне, не начиная даже первое
        обнаружение орфана/пропавшей ноги, пока не прошёл грейс-период.

        Зачем и для закрытия (реальный случай T 2026-09-15 05:xx): закрытие
        двух ног занимает 1-2с, ноги закрываются не одновременно. Сверка
        без пауз успела ДВАЖДЫ увидеть «bybit пуст, а в учёте T» ещё до
        того, как закрылась вторая нога и позиция снялась с учёта, —
        объявила «пропавшую ногу» и пошла закрывать «оставшуюся». Спасла
        только двойная проверка внутри _close_single_order (позиции уже не
        было — ордер не отправлен), но тревога в Telegram ушла ложная, а
        при чуть иных таймингах маркет-ордер «закрытия» несуществующей
        ноги открыл бы новую позицию в обратную сторону. Ровно та же
        гонка, что уже была на открытии (STORJ) — лечится тем же."""
    ...
    async def _reconcile_orphaned_positions(self) -> None:
        ...
        for coin, pos in list(tracked.items()):
            ...
            for leg_key, other_key, other_amount_key in (
                ...
                # НО СНАЧАЛА — свежая сверка с учётом (добавлено 2026-09-15,
                # случай T): `tracked` — снимок учёта на НАЧАЛО прогона. Если
                # за время прогона позицию легитимно закрыли и сняли с учёта
                # (сканером, вручную, сигналом канала), «пропавшая нога» — это
                # просто уже закрытая нога, и делать ничего не нужно. Это
                # универсальная защита для ЛЮБОГО пути закрытия, в отличие
                # от грейс-периода, который ставит только сканер.
                if position_store.get_position(coin_upper) is None:
                    self._pending_missing_legs.pop(key, None)
                    print(f"[reconcile] {coin_upper}: позиция уже снята с учёта за время прогона — нога не «пропала», а закрыта. Ничего не делаю.")
                    break
    ...
    async def _close_test_batch_position(
        ...
        # ГРЕЙС-ПЕРИОД ДЛЯ СВЕРКИ (см. _in_open_grace_period): пока ноги
