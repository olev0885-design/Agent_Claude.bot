...
# =============================================================================
def listen() -> None:
    # ЗАЩИТА ОТ ВТОРОГО БОТА (добавлено 2026-09-15, в день переезда на сервер).
    # Боевой бот работает на VPS в Токио. Если файл-маркер лежит в корне
    # проекта — эта машина НЕ должна торговать: одни ключи, один учёт, и два
    # бота закроют друг другу ноги как «неучтённые». Маркер ставится на
    # ноутбуке руками (`.bot_disabled_here`, в .gitignore) и не попадает на
    # сервер. Снять — удалить файл, но только если сервер остановлен.
    ...
    if os.path.exists(_marker):
        print(
            "[startup] СТОП: на этой машине бот отключён (маркер .bot_disabled_here). "
            "Боевой бот работает на сервере в Токио — два бота одновременно недопустимы. "
            "Чтобы запустить здесь, сначала остановите сервер и удалите маркер."
        )
        return
